...
public class GuiRedstoneJukebox extends GuiContainer {
    ...
    public void initGui() {
        ...
        this.buttonList.add(new GuiRedstoneJukeboxButtonLoop(0, this.guiLeft + 144, this.guiTop + 147));
        this.buttonList.add(new GuiRedstoneJukeboxButtonVolumeUp(1, this.guiLeft - 64, this.guiTop + 130));
        this.buttonList.add(new GuiRedstoneJukeboxButtonVolumeDown(2, this.guiLeft - 54, this.guiTop + 130));
        this.buttonList.add(new GuiRedstoneJukeboxButtonPlayMode(3, this.guiLeft + 124, this.guiTop + 126));
        this.buttonList.add(new GuiRedstoneJukeboxButtonPowerOn(4, this.guiLeft - 59, this.guiTop + 146));
        this.buttonList.add(new GuiRedstoneJukeboxButtonPrevious(5, this.guiLeft - 59, this.guiTop + 167));
        this.buttonList.add(new GuiRedstoneJukeboxButtonNext(6, this.guiLeft + 150, this.guiTop + 167));
    }
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        if (par1GuiButton.enabled) {
            ...
            case 4:
             ...
             /*   if (!this.jukeboxInventory.hasEnergy) {
                    this.jukeboxInventory.hasEnergy = true;
                    this.jukeboxInventory.isActive = true;
                    this.jukeboxInventory.checkRedstonePower();
                    this.jukeboxInventory.startPlaying();
                }
                else {
                    this.jukeboxInventory.hasEnergy = false;
                    this.jukeboxInventory.isActive = false;
                    this.jukeboxInventory.checkRedstonePower();
                    this.jukeboxInventory.stopPlaying();
                }  */
                break;
            ...
            case 5:
            	...
            	--slot;
                if (slot >= 0) {
                	this.jukeboxInventory.playRecord(slot);
                }
                else {
                	slot = this.jukeboxInventory.playOrder.length;
                }
            	break;
            ...
            case 6:
                // Play next record
            	++slot;
                if (slot <= this.jukeboxInventory.playOrder.length) {
                	this.jukeboxInventory.playRecord(slot);
                } 
                else {
                	slot = 0;
    ...
    public void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        this.drawTexturedModalRect(j - 78, k - 48, 0, 0, this.xSize + 80, this.ySize + 72);

        // -- loop indicator
        if (this.jukeboxInventory.isLoop) {
            // play loop
            this.drawTexturedModalRect(j + 144, k + 147, 19, 239, 18, 9);
        }
        else {
            // play once
            this.drawTexturedModalRect(j + 144, k + 147, 0, 239, 18, 9);
        }

        // -- power indicator
        if (!jukeboxInventory.hasEnergy) {
            // power off
            this.drawTexturedModalRect(j - 58, k + 145, 146, 239, 10, 10);
        }
        else {
            // power on
            this.drawTexturedModalRect(j - 58, k + 145, 157, 239, 10, 10);
        }
        ...
        case 0:
            // normal
            this.drawTexturedModalRect(j + 132, k + 127, 39, 239, 40, 10);
            break;

        case 1:
            // shuffle
            this.drawTexturedModalRect(j + 132, k + 127, 82, 239, 40, 10);
            break;
        }
