...
public class ContainerRedstoneJukebox extends Container {
    ...
    public ContainerRedstoneJukebox(InventoryPlayer inventoryPlayer, TileEntityRedstoneJukebox tileEntity) {
        ...
        for (int row = 0; row < 8; row++) {
          for (int col = 0; col < 13; col++) {
            addSlotToContainer(new SlotRedstoneJukeboxRecord(this.teJukebox, col + row * 13, -66 + col * 18, -40 + row * 18));
          }
        }
    ...
    protected void bindPlayerInventory(InventoryPlayer inventoryPlayer) {
        for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 9; j++) {
                        addSlotToContainer(new Slot(inventoryPlayer, j + i * 9 + 9, -30 + j * 18, 108 + i * 18));
                }
        }

        for (int i = 0; i < 9; i++) {
                addSlotToContainer(new Slot(inventoryPlayer, i, -30 + i * 18, 166));
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotnumber) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotnumber < 139) {
                ...
                if (!this.mergeItemStack(myStack, 0, 139, true)) return null;
            }
            else {
